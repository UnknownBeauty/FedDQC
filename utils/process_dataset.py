...
def get_dataset(dataset_name, local_data_dir=None):
    ...
    if dataset_name in ["gsm8k"]:
        ...
        dataset_name = local_data_dir + dataset_name if local_data_dir is not None else dataset_name
        dataset = load_dataset(dataset_name, field="train", split='train')
    return dataset
...
def process_val_dataset(dataset):
    instrution = "Your role as a doctor requires you to answer the medical questions taking into account the patient's description. Analyze the question given its context. Give yes/no decision."
    response_template = ('###Answer: {}')
    def map_val_set(example):
        example['instruction'] = instrution
        example['output'] = response_template.format(example['output'])
        return example
    dataset = dataset.map(map_val_set, load_from_cache_file=False)
    return dataset
